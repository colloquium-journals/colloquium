...
// Bot command interface for standalone package
...
// Command bot interface for standalone package  
...
const helpCommand: BotCommand = {
  name: 'help',
  description: 'Show detailed help for the reference bot',
  usage: '@reference-bot help',
  parameters: [],
  examples: ['@reference-bot help'],
  permissions: [],
  async execute(params, context) {
    let message = `📚 **Reference Bot Help**\n\n`;
    message += `I help you validate and check references in your manuscripts.\n\n`;
    
    message += `**Available Commands:**\n`;
    message += `• \`check-doi\` - Analyze all references for DOI presence and validity\n\n`;
    
    message += `**What I Check:**\n`;
    message += `✅ DOI presence in each reference\n`;
    message += `✅ DOI format validation\n`;
    message += `✅ DOI resolution (can the DOI be accessed?)\n`;
    message += `✅ Metadata retrieval from CrossRef\n\n`;
    
    message += `**Common Issues I Find:**\n`;
    message += `❌ References missing DOIs\n`;
    message += `❌ Malformed DOI formats\n`;
    message += `❌ DOIs that don't resolve to actual papers\n`;
    message += `❌ Broken or outdated DOI links\n\n`;
    
    message += `**Examples:**\n`;
    message += `• \`@reference-bot check-doi\` - Basic check\n`;
    message += `• \`@reference-bot check-doi detailed=true\` - Include paper metadata\n`;
    message += `• \`@reference-bot check-doi timeout=60\` - Extended timeout for large manuscripts\n\n`;
    
    message += `**Tips for Better References:**\n`;
    message += `💡 Always include DOIs when available\n`;
    message += `💡 Use the format "DOI: 10.xxxx/xxxx" or "https://doi.org/10.xxxx/xxxx"\n`;
    message += `💡 Verify DOIs work before submission\n`;
    message += `💡 For papers without DOIs, consider citing more recent versions if available`;
...
export const referenceBot: CommandBot = {
  ...
  version: '1.0.0',
  commands: [checkDoiCommand, helpCommand],
...
export const manifest = {
  name: '@colloquium/reference-bot',
  version: '1.0.0',
  description: 'Validates references and checks DOI availability and correctness for academic manuscripts',
  author: {
    name: 'Colloquium Team',
    email: 'team@colloquium.org',
    url: 'https://colloquium.org'
  },
  license: 'MIT',
  keywords: ['colloquium', 'bot', 'references', 'doi', 'citation', 'bibliography'],
  homepage: 'https://github.com/colloquium/colloquium',
  repository: {
    type: 'git' as const,
    url: 'https://github.com/colloquium/colloquium.git'
  },
  bugs: {
    url: 'https://github.com/colloquium/colloquium/issues'
  },
  colloquium: {
    botId: 'reference-bot',
    apiVersion: '1.0.0',
