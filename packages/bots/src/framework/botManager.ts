...
export class DatabaseBotManager implements BotManager {
  ...
  async install(source: BotInstallationSource, config: Record<string, any> = {}): Promise<BotInstallation> {
    try {
      // Load the plugin
      ...
      // Store package information in bot definition
      ...
      throw error instanceof BotPluginError ? error : new BotPluginError(
  ...
  async uninstall(botId: string): Promise<void> {
    ...
    if (!installation) {
      throw new BotPluginError(`Bot ${botId} is not installed`, 'NOT_INSTALLED');
    }
    ...
    try {
      ...
      if (!otherInstalls) {
        await prisma.botDefinition.delete({
          where: { id: botId }
        }).catch(() => {
  ...
  async update(botId: string, version?: string): Promise<BotInstallation> {
    ...
    try {
      ...
      throw new BotPluginError(
        `Failed to update bot ${botId}: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'UPDATE_FAILED',
        error
      );
    }
  ...
  async installDefaults(): Promise<BotInstallation[]> {
    ...
    const defaultBots = isDevelopment ? [
      ...
      { 
        source: { type: 'npm' as const, packageName: '@colloquium/plagiarism-bot' },
        config: { defaultThreshold: 0.15, enabledDatabases: ['crossref', 'pubmed', 'arxiv'] }
      },
      { 
        source: { type: 'npm' as const, packageName: '@colloquium/reference-bot' },
        config: { defaultTimeout: 30, includeMissingDoiReferences: true }
      }
    ];
  ...
  private extractBotIdFromPackageName(packageName: string): string {
    ...
    return packageName.replace('@colloquium/bot-', '').replace('@colloquium/', '');
  }
