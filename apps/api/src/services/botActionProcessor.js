...
exports.botActionProcessor = exports.BotActionProcessor = void 0;
...
class BotActionProcessor {
    ...
    async handleAssignReviewer(data, context) {
        const { reviewers, deadline, customMessage } = data;
        const { manuscriptId } = context;
        // Verify manuscript exists
        const manuscript = await database_1.prisma.manuscripts.findUnique({
        ...
        for (const email of reviewers) {
            try {
                ...
                let reviewer = await database_1.prisma.users.findUnique({
                    where: { email: email.toLowerCase() }
                });
                // If user doesn't exist, create them as a potential reviewer
                if (!reviewer) {
                    reviewer = await database_1.prisma.users.create({
                        data: {
                ...
                const existingAssignment = await database_1.prisma.review_assignments.findUnique({
                    where: {
                ...
                const assignment = await database_1.prisma.review_assignments.create({
                    data: {
                        manuscriptId,
                        reviewerId: reviewer.id,
                        status: 'PENDING',
...
Decline: ${invitationUrl}?action=decline
            `
                ...
                await (0, events_1.broadcastToConversation)(context.conversationId, {
    ...
    async handleUpdateManuscriptStatus(data, context) {
        ...
        const currentManuscript = await database_1.prisma.manuscripts.findUnique({
            where: { id: manuscriptId },
            select: { status: true }
        });
        ...
        const updatedManuscript = await database_1.prisma.manuscripts.update({
            where: { id: manuscriptId },
            data: {
                status,
                updatedAt: new Date()
        ...
        await database_1.prisma.messages.create({
            data: {
    ...
    async handleCreateConversation(data, context) {
        ...
        for (const participantId of uniqueParticipants) {
            await database_1.prisma.conversation_participants.create({
                data: {
    ...
    async handleRespondToReview(data, context) {
        const { assignmentId, response, message } = data;
        const { userId } = context;
        // Find the review assignment
        const assignment = await database_1.prisma.review_assignments.findUnique({
        ...
        await database_1.prisma.review_assignments.update({
            where: { id: assignmentId },
            data: { status: newStatus }
        });
        // Create notification message in editorial conversation
        const editorialConversation = await database_1.prisma.conversations.findFirst({
            where: {
    ...
    async handleSubmitReview(data, context) {
        ...
        const reviewConversation = await database_1.prisma.conversations.findFirst({
            where: {
                manuscriptId: assignment.manuscriptId,
                type: 'REVIEW'
            }
    ...
    async handleMakeEditorialDecision(data, context) {
    ...
    async sendDecisionEmail(email, manuscript, decision, conversationId) {
...
${isPositive ? 'Congratulations! Your manuscript has been accepted for publication.\n' : ''}
    ...
    async createRevisionConversation(manuscriptId, editorId, revisionType) {
        // Check if revision conversation already exists
        const existingConversation = await database_1.prisma.conversations.findFirst({
            where: {
                manuscriptId,
    ...
    async handleAssignActionEditor(data, context) {
        ...
        if (manuscript.action_editors) {
            ...
            const updatedAssignment = await database_1.prisma.action_editors.update({
                where: { manuscriptId },
                data: {
                    editorId: user.id,
                    assignedAt: new Date()
            ...
            const newAssignment = await database_1.prisma.action_editors.create({
                data: {
...
exports.BotActionProcessor = BotActionProcessor;
exports.botActionProcessor = new BotActionProcessor();
